...
const removeColorVariant = async (color) => {
  ...
  const hasVariantId = variant.variantId && variant.variantId !== 'undefined' && variant.variantId !== 'null';
  
  if (hasVariantId) {
    // It's an existing variant - delete from backend
    ...
    // New variant not saved to backend yet
    cleanupVariantResources(variant);
    ...
    toast.info(`Variant "${color}" removed from form`);
  }
};

// Helper function to clean up image URLs
const cleanupVariantResources = (variant) => {
