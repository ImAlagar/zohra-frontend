...
const RelatedProducts = ({ currentProduct, category }) => {
  ...
  const { data: relatedProductsData, isLoading, error } = useGetRelatedProductsQuery({
    category,
    excludeProductId: currentProduct?._id || currentProduct?.id
  }, {
    skip: !category || !currentProduct
  });
  ...
  const splitProductsByColor = (apiProduct) => {
    ...
    return Object.entries(colorGroups).map(([color, colorData]) => {
      ...
      // Get the correct product ID (slug or _id)
      const productId = apiProduct.slug || apiProduct._id || apiProduct.id;
      
      return {
        id: productId, // Use the original product ID, not appended with color
  ...
  if (relatedProductsData) {
    ...
    const colorBasedProducts = productsArray
      .filter(product => {
        ...
        const currentProductId = currentProduct?.slug || currentProduct?._id || currentProduct?.id;
